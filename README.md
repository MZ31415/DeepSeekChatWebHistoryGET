...
5.  **输出**：最终，它会把聊天记录打印在控制台并复制到剪贴板，或通过 HTTP POST 请求将数据发送至本地运行的 Python 服务器进行保存。
...
此步骤是可选的。如果您只想在浏览器控制台查看记录或在剪贴板得到原始JSON，可以跳过这一步。但本服务器代码可以提供文件保存（JSON和Markdown格式）。
...
7.  **提取结果**：
    - **如果 Python 服务器未运行**：提取的聊天记录文本会在浏览器的开发者工具控制台（按F12 打开）中打印出一个格式化的对象数组，您可以直接在此查看。同时原始的JSON内容会复制到剪贴板。
...
- 得到的聊天记录只有**当前**显示的时间线，**不会**得到分支的线。如在此图显示下，只会得到消息2，不会得到消息1。 
    ![对话分支图片](./1.png)

- 如果AI正在流式输出消息，此时尝试提取聊天记录时，该消息不会被记录。

- 上百条聊天记录消息提取时可能较慢，实测378条消息大概花费30s。
